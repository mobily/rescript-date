...
describe("differenceInYears", () => {
  ...
  test("the difference is less than a year, but the given dates are in different calendar years", () => {
    let fstDate = makeWithYMD(~year=2019., ~month=0., ~date=1., ());
    let sndDate = makeWithYMD(~year=2018., ~month=1., ~date=1., ());

    fstDate->ReDate.differenceInYears(sndDate)->expect->toEqual(0, _);
  });
  ...
  test("the days and months of the given dates are the same", () => {
    ...
    fstDate->ReDate.differenceInYears(sndDate)->expect->toEqual(2, _);
  });
  ...
  test("returns the number of full years between the given dates", () => {
    ...
    fstDate->ReDate.differenceInYears(sndDate)->expect->toEqual(3, _);
  });
