...
describe("differenceInWeeks", () => {
  ...
  test("returns 0 if the difference is less than a week", () => {
    ...
    fstDate->ReDate.differenceInWeeks(sndDate)->expect->toEqual(0, _);
  });
  ...
  test("returns the number of full weeks between the given dates", () => {
    ...
    let fstDate = makeWithYMDHMS(~year=2018., ~month=0., ~date=10., ~hours=10., ~minutes=15., ~seconds=55., ());
    let sndDate = makeWithYMDHMS(~year=2018., ~month=0., ~date=2., ~hours=20., ~minutes=50., ~seconds=10., ());

    fstDate->ReDate.differenceInWeeks(sndDate)->expect->toEqual(1, _);
  });

  test("returns a negative number if the time value of the first date is smaller", () => {
    let fstDate = makeWithYMDHMS(~year=2018., ~month=0., ~date=2., ~hours=20., ~minutes=50., ~seconds=10., ());
    let sndDate = makeWithYMDHMS(~year=2018., ~month=0., ~date=10., ~hours=10., ~minutes=15., ~seconds=55., ());
