...
describe("differenceInWeeks", () => {
  ...
  test("returns the number of calendar weeks between the given dates", () => {
    ...
    fstDate
    |> ReDate.differenceInCalendarWeeks(sndDate)
    |> expect
    |> toEqual(6);
  });
  ...
  test("allows to specify which day is the first day of the week", () => {
    ...
    let differenceInCalendarWeeks' =
      ReDate.differenceInCalendarWeeks(~weekStartsOn=Monday);

    fstDate |> differenceInCalendarWeeks'(sndDate) |> expect |> toEqual(3);
  });
