...
describe("getOverlappingDaysInIntervals", () => {
  ...
  test(
    "returns the correct value for an interval included within another interval",
    () => {
    ...
    interval
    |> ReDate.getOverlappingDaysInIntervals(includedInterval)
    |> expect
    |> toEqual(10);
  });

  test(
    "returns the correct value for an interval including another interval", () => {
    ...
    includingInterval
    |> ReDate.getOverlappingDaysInIntervals(interval)
    |> expect
    |> toEqual(5);
  });

  test(
    "returns 0 for a valid non overlapping interval before another interval",
    () => {
    let earlierInterval = {
  ...
  test(
    "returns 0 for a valid non overlapping interval after another interval", () => {
    let laterInterval = {
